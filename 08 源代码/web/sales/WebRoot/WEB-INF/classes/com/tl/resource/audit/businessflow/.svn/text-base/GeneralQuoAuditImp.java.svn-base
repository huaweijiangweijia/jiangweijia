...
public class GeneralQuoAuditImp implements IAuditBusinessObject,IAuditContentBusinessObject {
	...
	private String businessType = "001";
	@Override
	...
	public String getUrl() {
		// TODO Auto-generated method stub
		return "/js/quotation/generalQuo/QuoDetailWindow.js";
	}
	...
	public void loadBo(String id) {
		...
		bo.addProperty("url", "/js/quotation/generalQuo/QuoDetailWindow.js");
	}
	...
	public List<TAuditContentDefDto> getBusinessAuditContents(String id) {
		...
		boolean allDeliveryed = generalQuoService.isAllDeliveryed(id);
		List<TAuditContentDefDto> list = auditContentService.findAllAuditContentList();
		if(change && hasNonStand && closingAccountModeChange && !allDeliveryed) return list;
		...
		for (Iterator iterator = list.iterator(); iterator.hasNext();) {
			...
			if(!change && PRICE.equals(auditContentDefDto.getId())){
				...
				continue;
			}else if(allDeliveryed && DELIVERY_TYPE.equals(auditContentDefDto.getId())){
				continue;
			}
