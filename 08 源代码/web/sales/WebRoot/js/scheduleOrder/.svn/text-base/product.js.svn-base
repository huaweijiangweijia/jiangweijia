	...
	var qOContractDetailTree_Check = new Ext.grid.CheckboxSelectionModel();
	var qOContractDetailTree =  Ext.extend(Ext.grid.GridPanel,{
		contractCode:null,
		supplierId:null,
		productBrand:null,
		store:null,
...
//			this.store.baseParams.productBrand = this.productBrand;
			...
			qOContractDetailTree.superclass.constructor.call(this, {
				height:520,
				sm:qOContractDetailTree_Check,
				store:this.store,
				columns:[
					 new Ext.grid.RowNumberer(),
					qOContractDetailTree_Check,
...
var  qOProductSelectForm = Ext.extend(Ext.FormPanel, {
	contractCode:null,
	store:null,
	constructor : function(_cfg) {
		if(_cfg == null) {
		...
		this.store.baseParams.quotationCode = this.contractCode;
		this.store.load();
		qOProductSelectForm.superclass.constructor.call(this, {
	...
	var qOProductPanel = Ext.extend(Ext.Panel,{
		detail_list:null,
		contractCode:null,
		supplierId:null,
		selectForm2:null,
		...
		constructor : function(_cfg){
			...
			Ext.apply(this, _cfg);
			this.selectForm2 = new qOProductSelectForm({contractCode:this.contractCode});
			this.detail_list = new qOContractDetailTree({contractCode:this.contractCode,supplierId:this.supplierId,productBrand:this.productBrand});
			qOProductPanel.superclass.constructor.call(this, {
				iconCls:'icon-grid',
				split: true,
				width: 200,
				height : 500,
	...
	var qOProductWin = Ext.extend(Ext.Window,{
		nav2:null,
		contractCode:null,
		supplierId:null,
		productBrand:null,
		...
		constructor : function(_cfg){
			...
			Ext.apply(this, _cfg);
			this.nav2 = new qOProductPanel({contractCode:this.contractCode,supplierId:this.supplierId,productBrand:this.productBrand});
			...
			qOProductWin.superclass.constructor.call(this, {
				title:"添加产品",  
				width:800,  
				height:550,  
//				modal:true,
...
//									_grid.baseParams.searchStr = Ext.encode(searchStr);
				 ...
				 buttons : [{
					text : "确定",
					handler : function() {
						var arr = qOContractDetailTree_Check.getSelections();
						if(arr.length < 1) {
							Ext.Msg.show({
								title:'系统提示',
								msg: '请选择一条记录进行操作!',
