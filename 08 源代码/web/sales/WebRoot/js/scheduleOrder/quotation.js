	...
	var qOContractGrid =  Ext.extend(Ext.grid.GridPanel,{
		store:null,
		constructor : function(_cfg){
			...
			Ext.apply(this, _cfg);
			var proxy = new Ext.data.HttpProxy({url: PATH + "/scheduleOrder/quotationList.do?quotationType=3&leaf=1&outStockType=5"});
			...
			qOContractGrid.superclass.constructor.call(this, {
				height:520,
				store:this.store,
				columns: [
...
function setRowLockStatus(grid){
	...
	var qOContractTree =  Ext.extend(Ext.grid.GridPanel,{
		qId:null,
		store:null,
		constructor : function(_cfg){
			if(_cfg == null) {
			...
			qOContractTree.superclass.constructor.call(this, {
				height:520,
				store:this.store,
				columns: [
					 new Ext.grid.RowNumberer(),
...
var  qOContractSelectForm = Ext.extend(Ext.FormPanel, {
	constructor : function(_cfg) {
		if(_cfg == null) {
			_cfg = {};
		}
		Ext.apply(this, _cfg);
		qOContractSelectForm.superclass.constructor.call(this, {
	...
	var qOContractWin = Ext.extend(Ext.Window,{
		contract_list:null,
		selectForm2:null,
		qOContractTree:null,
		constructor : function(_cfg){
			...
			Ext.apply(this, _cfg);
			this.qOContractTree = new qOContractTree();
			this.selectForm2 = new qOContractSelectForm();
			this.contract_list = new qOContractGrid();
			var record = null;
			this.contract_list.on('rowclick',function(grid,rowIndex,e) {
						var s = grid.getStore();
						record = s.getAt(rowIndex);
						this.qOContractTree.store.setBaseParam('qId',record.id);
						this.qOContractTree.store.load({params:{start:0,limit:20,id:record.id}});
					},this);
			this.contract_list.on('rowdblclick',function(grid,rowIndex,e)
			{
				...
				if(cusLockStatus * 1 == 0){
					Ext.Msg.alert("消息", "所选中预定报价单,所属客户已经被锁定,不允许做订单!");
					return ;
				}
				try{
						Ext.Ajax.request({
							...
							success: function(response){
									...
									if(detail.length == 0){
										...
										var win = new qOSupplierWin({quotationId:contractInforId,deliveryAddressType:record.get('deliveryType'),quotationId:record.get('id'),
									...
									{
										outStockInfor:{quotationId:contractInforId,userName:LoginInfor.user.trueName,customerCode:customerCode,customerName:customerName,outStockType:5,status:0,quotationCode:quotationCode,outStockDetails:detail},
												...
												Ext.Ajax.request({
													...
													success: function(response){
														eval('var temp='+response.responseText);
															if(temp.length > 0){
																var win = new qOSupplierWin({quotationId:contractInforId,deliveryAddressType:record.get('deliveryType'),
																quotationCode:record.get('quotationCode'),ownContactPerson:record.get('userName'),customerName:customerName,customerCode:customerCode});
																var store = win.supplier_grid.getStore();
																store.baseParams.qId = record.get('id');      
																store.baseParams.leaf = 1;      
...
//			this.qOContractTree.on('rowdblclick',function(grid,rowIndex,e) {
//				var s = grid.getStore();
//				var r = s.getAt(rowIndex);
//				var quotationCode = record.get("quotationCode");
//				var customerName = record.get("customerName");
...
//									if(detail.length == 0){
//										//Ext.Msg.alert("消息", "所选合同明细中，没有产品可提取库存,确定后将编制采购订单!");
//										var win = new qOSupplierWin({deliveryAddressType:record.get('deliveryType'),quotationCode:record.get('quotationCode'),node:r,ownContactPerson:record.get('userName')});
//										var store = win.supplier_grid.getStore();
//										store.baseParams.qId = record.get('id');  
...
//				this.close();  
//			},this);
			qOContractWin.superclass.constructor.call(this, {
...
//					var _record  = this.qOContractTree.getSelectionModel().getSelected();      
						if (1!=selectedItem) {                                                  
							Ext.Msg.alert('系统提示', '请选择一条报价单记录!');                       
							return;                                                             
						}else                                                                   
						{   
							var cusLockStatus = record.get("cusLockStatus");
