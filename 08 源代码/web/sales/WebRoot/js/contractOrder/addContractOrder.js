...
/**添加合同订单多选框**/
Ext.ls.contractOrder.addChek = new Ext.grid.CheckboxSelectionModel();

/**添加产品**/
Ext.ls.contractOrder.add = function(btn,e){
	var grid = btn.ownerCt.ownerCt;
	var store = grid.store;
	var win = new cOProductWin({contractCode:grid.contractCode,supplierId:grid.supplierId,
...
/**删除产品**/
Ext.ls.contractOrder.deleteProduct = function(btn,e){
	var grid = btn.ownerCt.ownerCt;
	var store = grid.getStore();
	var arr = Ext.ls.contractOrder.addChek.getSelections();
	var array = [];
	if(arr.length < 1) {
...
/**统一设置交货期限**/
Ext.ls.contractOrder.setupDate = function(btn,e)
{
	var grid = btn.ownerCt.ownerCt;
	var store = grid.getStore();
...
/**历史价格查询**/
Ext.ls.contractOrder.queryHistory = function(btn,e)
{
	var grid = btn.ownerCt.ownerCt;
	var store = grid.getStore();
	try
...
/**合同订单详细grid**/
	Ext.ls.contractOrder.addGird =  Ext.extend(Ext.grid.EditorGridPanel,{
		contractCode:null,
		supplierId:null,
		productBrand:null,
		contractId:null,
		...
		fields : ['id','brandCode','productCode','productName','productUnit','contractAmount',
			...
			'contractProjectSortId','proSortName','projectCode','serialNumber','orderAmount','productMoney','deliveryDate','priceChange']});
		/**给store传参**/
		...
		Ext.ls.contractOrder.addGird.superclass.constructor.call(this, {
			layout: 'fit',
			clicksToEdit: 1,
			frame:true,
			sm:Ext.ls.contractOrder.addChek,
			store:this.store,
			columns:[
					 new Ext.grid.RowNumberer(),
					Ext.ls.contractOrder.addChek,
					{header:'合同订单ID',width:100,dataIndex:'id',hidden:true},
					{header:'合同分名称',width:100,dataIndex:'proSortName',sortable:true},
					{header:'项目号',width:50,dataIndex:'projectCode',sortable:true},
					...
					{header:'合同剩余数量',width:120,dataIndex:'remainAmount',sortable:true},
			...
			tbar :	[{text : "添加",iconCls:'icon-add',handler :Ext.ls.contractOrder.add,scope : this,hidden:this.tbarHidden},
					{xtype:'tbseparator',hidden:this.tbarHidden},
					{text : "删除",iconCls : 'icon-delete',handler : Ext.ls.contractOrder.deleteProduct,scope:this,hidden:this.tbarHidden},
					{xtype:'tbseparator',hidden:this.tbarHidden},
					{text : '统一设置交货期限',iconCls : 'icon-date',handler :Ext.ls.contractOrder.setupDate,scope : this,hidden:this.tbarHidden},
					{xtype:'tbseparator',hidden:this.tbarHidden},
					{text : '历史价格查询',iconCls : 'icon-date',handler :Ext.ls.contractOrder.queryHistory,scope : this,hidden:this.tbarHidden}],
			/**分页信息，当页面显示明细信息时显示**/
...
/**添加订单form**/
	Ext.ls.contractOrder.addForm = Ext.extend(Ext.FormPanel,{
		detailFlag:null,//是否明细页面
...
//			this.status = new StatusCombox({disabled : true,x:100,y:93,width:170,value:0});
			...
			var config = [
				//
					...
					{xtype:'label',text: '合同编号:',x:530,y:5,style:this.lableStyle_},
					{xtype:'textfield',  name: 'contractCode',readOnly : true,x:630,y:2,width:170},
...
//					{xtype:'textfield',  name: '',value:'编制',readOnly : true,x:100,y:93,width:170},
			...
			Ext.ls.contractOrder.addForm.superclass.constructor.call(this, {
					width : 1000,
	...
	Ext.ls.contractOrder.addWin = Ext.extend(Ext.Window,{
		grid:null,
		form:null,
		deliveryAddressType:null,//销售合同交货地点及方式
		contractId:null,
		contractCode:null,
		supplierId:null,
		...
		constructor : function(_cfg){
			...
			Ext.apply(this, _cfg);
			this.form = new Ext.ls.contractOrder.addForm({detailFlag:this.detailFlag});
			this.grid = new Ext.ls.contractOrder.addGird({contractId:this.contractId,contractCode:this.contractCode,supplierId:this.supplierId,detailFlag:this.detailFlag,URL:this.URL,orderId:this.orderId});
			if(this.updateFlag){this.updateFlag = false,this.detailFlag = true}
			else{this.updateFlag = true}//修改页面
			Ext.ls.contractOrder.addWin.superclass.constructor.call(this, {
						...
						maximizable: true,
						layout:"border",  
						bbar :[{xtype:'label',html : "销售合同交货地点及运输方式:<font color = 'red'>"+this.deliveryAddressType+"<font>"}],
						buttons : [{
							text : "保存",
							hidden:this.detailFlag,
							handler : function() {
								var formV = this.form.getForm().getValues();
								if(!checkOrderInfor(formV,this.form))
								{return;}
								...
								url: PATH + '/contractOrder/addOrder.do',
									params: { order: order, orderDetail : Ext.encode(orderDetail),orderType : 1},
									success : function(response) {
										var responseArray = Ext.util.JSON.decode(response.responseText); 
											 if(responseArray.success == true){
												...
												//添加成功后自动跳转到更新页面
												//更改页面标题
												this.setTitle('修改合同订单');
												var grid = this.grid;
												var store = new Ext.data.JsonStore({
													url:Ext.ls.contractOrder.updateUrl,
													remoteSort : true,
													totalProperty : 'totalProperty',
													root : 'root',
													fields : ['id','brandCode','productCode','productName','productUnit','contractAmount',
...
//												 this.close();
												 var ffcStore = cut_tools.contract_order.index_grid.getStore();
												...
												success : function(_response) {
													var _responseArray = Ext.util.JSON.decode(_response.responseText); 
													 if(_responseArray.success == true){
														 Ext.MessageBox.show({  
														  title : "价格提示",  
														  msg:"序列号为:"+ _responseArray.numStr+"产品的采购价格高于合同中销售价格",  
														  width : 300,  
   ...
   this.show = function(){
       if(this.method != null){
	       this.method();
			...
			store.load();
			/**订单修改页面**/
			var win = new Ext.ls.contractOrder.addWin({orderId:this.id,detailFlag:true,URL:Ext.ls.contractOrder.detailUrl});
			/**设置窗口的标题**/
