...
/**添加产品**/
Ext.ls.trySelfOrder.add = function(btn,e){
...
/**删除产品**/
Ext.ls.trySelfOrder.deleteProduct = function(btn,e){
	var grid = btn.ownerCt.ownerCt;
...
/**统一设置交货期限**/
Ext.ls.trySelfOrder.setupDate = function(btn,e)
{
	var grid = btn.ownerCt.ownerCt;	
...
/**试刀加工订单详细grid**/
Ext.ls.trySelfOrder.addGrid = Ext.extend(Ext.tree.ColumnTree,{
		quotationCode:null,
		supplierId:null,
		quotationId:null,
		URL:null,//store的URL
		...
		constructor : function(_cfg){
			...
			Ext.ls.trySelfOrder.addGrid.superclass.constructor.call(this, {
					    width:900,
						height: 270,
						rootVisible:false,
						autoScroll:true,
			...
			tbar :	[{text : "添加",iconCls:'icon-add',handler :Ext.ls.trySelfOrder.add,scope : this,hidden:this.tbarHidden},
					{xtype:'tbseparator',hidden:this.tbarHidden},
					{text : "删除",iconCls : 'icon-delete',handler : Ext.ls.trySelfOrder.deleteProduct,scope:this,hidden:this.tbarHidden},
					{xtype:'tbseparator',hidden:this.tbarHidden},
					{text : '统一设置交货期限',iconCls : 'icon-date',handler :Ext.ls.trySelfOrder.setupDate,scope : this,hidden:this.tbarHidden}],
...
Ext.ls.trySelfOrder.addForm = Ext.extend(Ext.FormPanel,{
		constructor : function(_cfg){
			if(_cfg == null) {
				_cfg = {};
			}
			...
			var config = [
				//
					...
					{xtype:'numberfield',  name: 'productMoney',readOnly : true,x:630,y:3,width:170},
					//2
...
//					{xtype:'textfield',  name: 'status',value:'编制',readOnly : true,x:100,y:62,width:170},
					...
					{xtype:'numberfield',  name: 'taxMoney',readOnly : true,x:630,y:63,width:170},
					//4
					...
					{xtype:'numberfield',  name: 'totalMoney',readOnly : true,x:630,y:93,width:170},
					//5
					{xtype:'label',text: '交货日期:',x:0,y:125,style:this.lableStyle_},
					this.deliveryDate,
					{xtype:'label',text: '最终金额:',x:530,y:125,style:this.lableStyle_},
					{xtype:'numberfield', name: 'finalMoney', allowBlank : false, x:630,y:122,width:170},
					//6
			...
			Ext.ls.trySelfOrder.addForm.superclass.constructor.call(this, {
				width : 1000,
...
Ext.ls.trySelfOrder.addWin = Ext.extend(Ext.Window,{
		grid:null,
		form:null,
		quotationId:null,
		quotationCode:null,
		...
		constructor : function(_cfg){
			...
			Ext.apply(this, _cfg);
			this.grid = new Ext.ls.trySelfOrder.addGrid({quotationId:this.quotationId,quotationCode:this.quotationCode,supplierId:this.supplierId,detailFlag:this.detailFlag,URL:this.URL,orderId:this.orderId});
			this.form = new Ext.ls.trySelfOrder.addForm();
			...
			Ext.ls.trySelfOrder.addWin.superclass.constructor.call(this, {
						...
						buttons : [{
							...
							handler : function() {
								...
								Ext.Ajax.request({
								url: PATH + '/selfOrder/addOrder.do?orderType=8',
									...
									success : function(response) {
										var responseArray = Ext.util.JSON.decode(response.responseText); 
										 ...
										 cut_tools.ts_order.index_grid.getStore().reload();
										 } else {
   ...
   this.show = function(){
       if(this.method != null){
	       this.method();
			...
			store.load();
			/**订单修改页面**/
			var win = new Ext.ls.trySelfOrder.addWin({orderId:this.id,detailFlag:true,URL:Ext.ls.trySelfOrder.detailUrl});
			/**设置窗口的标题**/
			win.setTitle('查看试刀加工订单明细');
			/**将store的数据加载到页面**/
