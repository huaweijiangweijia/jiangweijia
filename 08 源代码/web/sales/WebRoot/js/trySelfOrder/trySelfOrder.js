...
cut_tools.ts_order.index_grid = null;	    
...
Ext.apply(_config, getConfig());
	var store = new Ext.data.JsonStore({
				url : PATH + "/scheduleOrder/orderListAction.do?orderType=8",
				remoteSort : true,
				...
				fields : ['id',"orderCode","quotationCode","currencyId","currencyName","orderDate","orderType","status","supplierName","supplierId",
					...
					"overallRebate","finalMoney",'fileCount']
			});
	...
	cut_tools.ts_order.index_grid = new Ext.grid.GridPanel({
		frame:true,
		layout: 'fit',
		sm:gridCheckSele,
		store: store,
...
//			},
			{header: "订单附件", width: 60,  dataIndex: 'fileCount',
			 ...
			 tbar : [{
				...
				listeners: {
					'click' : function(){
						var record  = cut_tools.ts_order.index_grid.getSelectionModel().getSelected();
						...
						/**订单修改页面**/
						var win = new  Ext.ls.trySelfOrder.addWin({orderId:record.get('id'),quotationCode:record.get('quotationCode'),supplierId:record.get('supplierId'),updateFlag:true,URL:Ext.ls.trySelfOrder.updateUrl});
						/**订单信息**/
						var store = new contractOrderStore();
						store.baseParams.orderId = record.get('id');
						store.load();
						store.on('load',function(){
							win.setTitle('修改试刀加工订单');
				...
				listeners: {
					'click' : function(){
							...
							var handleDelete = function (btn){
								var ds = cut_tools.ts_order.index_grid.getStore() 
								if(btn == 'ok') {	
									Ext.Ajax.request({
										method: "post",
...
//							}
//							Ext.Ajax.request({
//										url: PATH + '/trySelfOrder/submitAudit.do',
//										params: { ids: ids },
//											success: function(response){
...
//			},
{
				...
				listeners: {
					'click' : function(){
						...
						/**订单修改页面**/
						var win = new  Ext.ls.trySelfOrder.addWin({orderId:arr[0].id,detailFlag:true,URL:Ext.ls.trySelfOrder.updateUrl});
						/**订单信息**/
						var store = new contractOrderStore();
				...
				listeners: {
					'click' : function(){	
						...
						window.open(PATH + '/contractOrder/orderListExcel.do?orderType=8&searchForm='+Ext.util.JSON.encode(record));
					}
		 		}
			},{
				xtype:'tbseparator',
	...
	var  selectForm2 = new Ext.FormPanel({
	        ...
	        items:[
	           ...
	           bbar : ['->',{
			           		...
			           		iconCls : 'icon-search',
			           		handler : function() {
								var store = cut_tools.ts_order.index_grid.store;
	           ...
	           items:[
				{items: [{xtype:'textfield',fieldLabel: '订单编号',name: 'orderCode',anchor:'90%'},{xtype:'hidden',fieldLabel: '订单类型',name: 'orderType',value:8}]},
...
//					{items: [{xtype:'textfield',fieldLabel: '合同我方负责人',name: 'ownContactPerson',anchor:'90%'}]},
	...
	cut_tools.ts_order.index_win = new Ext.Panel({
            layout: 'border',
            ...
            items: [
            {
				...
				layout: 'fit',
                margins: '-5 5 5 5',
                items : [cut_tools.ts_order.index_grid]
            }],
		...
		cut_tools.ts_order.index_win.render('trySelf_order');
