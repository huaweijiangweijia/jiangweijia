...
/**添加合同订单多选框**/
Ext.ls.tryOrder.addChek = new Ext.grid.CheckboxSelectionModel();

/**添加产品**/
Ext.ls.tryOrder.add = function(btn,e){
	var grid = btn.ownerCt.ownerCt;
	var store = grid.store;
	var win = new tOProductWin({contractCode:grid.quotationCode,supplierId:grid.supplierId,
...
/**删除产品**/
Ext.ls.tryOrder.deleteProduct = function(btn,e){
	var grid = btn.ownerCt.ownerCt;
	var store = grid.getStore();
	var arr = Ext.ls.tryOrder.addChek.getSelections();
	var array = [];
	if(arr.length < 1) {
...
/**统一设置交货期限**/
Ext.ls.tryOrder.setupDate = function(btn,e)
{
	var grid = btn.ownerCt.ownerCt;
	var store = grid.getStore();
...
/**历史价格查询**/
Ext.ls.tryOrder.queryHistory = function(btn,e)
{
	var grid = btn.ownerCt.ownerCt;
	var store = grid.getStore();
	try
...
/**试刀订单详细grid**/
Ext.ls.tryOrder.addGrid =  Ext.extend(Ext.grid.EditorGridPanel,{
		quotationCode:null,
		supplierId:null,
		store:null,
		quotationId:null,
		...
		,'serialNumber','orderAmount','productMoney','deliveryDate','priceChange']});
		/**给store传参**/
		...
		Ext.ls.tryOrder.addGrid.superclass.constructor.call(this, {
			layout: 'fit',
			clicksToEdit: 1,
			frame:true,
			...
			tbar :	[{text : "添加",iconCls:'icon-add',handler :Ext.ls.tryOrder.add,scope : this,hidden:this.tbarHidden},
					{xtype:'tbseparator',hidden:this.tbarHidden},
					{text : "删除",iconCls : 'icon-delete',handler : Ext.ls.tryOrder.deleteProduct,scope:this,hidden:this.tbarHidden},
					{xtype:'tbseparator',hidden:this.tbarHidden},
...
/**试刀订单form**/
Ext.ls.tryOrder.addForm = Ext.extend(Ext.FormPanel,{
		constructor : function(_cfg){
			if(_cfg == null) {
				_cfg = {};
			}
			...
			var config = [
				//
					...
					{xtype:'textfield',  name: 'ownContactPerson',readOnly : false,x:630,y:33,width:170},
...
//					{xtype:'textfield',  name: 'status',value:'编制',readOnly : true,x:100,y:93,width:170},
			...
			Ext.ls.tryOrder.addForm.superclass.constructor.call(this, {
					width : 1000,
	...
	Ext.ls.tryOrder.addWin = Ext.extend(Ext.Window,{
		grid:null,
		form:null,
		quotationId:null,
		quotationCode:null,
		...
		constructor : function(_cfg){
			...
			Ext.apply(this, _cfg);
			this.grid =  new Ext.ls.tryOrder.addGrid({quotationId:this.quotationId,quotationCode:this.quotationCode,supplierId:this.supplierId,detailFlag:this.detailFlag,URL:this.URL,orderId:this.orderId});
			this.form = new Ext.ls.tryOrder.addForm();
			if(this.updateFlag){this.updateFlag = false,this.detailFlag = true}
			else{this.updateFlag = true}//修改页面
			Ext.ls.tryOrder.addWin.superclass.constructor.call(this, {
						...
						buttons : [{
							...
							handler : function() {
								...
								url: PATH + '/contractOrder/addOrder.do',
									params: { order: order, orderDetail : Ext.encode(orderDetail) ,orderType : 6},
									success : function(response) {
										var responseArray = Ext.util.JSON.decode(response.responseText); 
											 if(responseArray.success == true){
												...
												var store = new Ext.data.JsonStore({
													...
													fields : ['id','brandCode','productCode','productName','productUnit','contractAmount',
														'remainAmount','productBrand','price','parentToolsId','toolsId','leaf','contractProductDetailId',
														'serialNumber','orderAmount','productMoney','deliveryDate']});
												...
												var ffcStore = cut_tools.t_order.index_grid.getStore();
													...
													success : function(_response) {
														var _responseArray = Ext.util.JSON.decode(_response.responseText); 
														 if(_responseArray.success == true){
																  Ext.MessageBox.show({  
																  title : "价格提示",  
																  msg:"序列号为:"+ _responseArray.numStr+"产品的采购价格高于试刀申请中销售价格",  
																  width : 300,  
																  ...
																  fn : function(_btn) {  
																	 ...
																	 cut_tools.t_order.index_grid.getStore().reload();
																  }  
																});  
														 }else{
														     Ext.Msg.show({title:'成功提示',msg: responseArray.msg,buttons: Ext.Msg.OK,icon: Ext.MessageBox.INFO});
   ...
   this.show = function(){
       if(this.method != null){
	       this.method();
			...
			store.load();
			/**订单修改页面**/
			var win = new Ext.ls.tryOrder.addWin({orderId:this.id,detailFlag:true,URL:Ext.ls.tryOrder.detailUrl});
			/**设置窗口的标题**/
			win.setTitle('查看试刀订单明细');
			/**将store的数据加载到页面**/
