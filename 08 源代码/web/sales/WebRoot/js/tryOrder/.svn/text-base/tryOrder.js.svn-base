...
cut_tools.t_order.index_grid = null;    
Ext.onReady(function(){
	var PAGESIZE = parseInt((Ext.getBody().getHeight()-320)/24);
	var proxy = new Ext.data.HttpProxy({url: PATH + "/scheduleOrder/orderListAction.do?orderType=6"});
    function insertTotalRow(){
		var arr = cut_tools.t_order.index_grid.getView().getRows();
		if(arr.length){
...
Ext.apply(_config, getConfig());
	...
	cut_tools.t_order.index_grid = new Ext.grid.GridPanel({
		frame:true,
		layout: 'fit',
		sm:qgridCheckSele,
		store: store,
...
//			},
			 ...
			 tbar : [
			{
				...
				listeners: {
					'click' : function(){
						var win = new tOContractWin();
						win.show();
						win.maximize();
						win.contract_list.getStore().load({params: {start: 0, limit: 20}});
					}
				}
			},{
				xtype:'tbseparator',
				...
				listeners: {
					'click' : function(){
					 var arr = qgridCheckSele.getSelections();
						...
						store.load();
						/**订单修改页面**/
						var win = new Ext.ls.tryOrder.addWin({orderId:arr[0].id,detailFlag:true,URL:Ext.ls.tryOrder.detailUrl});
						/**设置窗口的标题**/
				...
				listeners: {
					'click' : function(){
						var record  = cut_tools.t_order.index_grid.getSelectionModel().getSelected();
						var arr = qgridCheckSele.getSelections();
						if(arr.length == 0){
							Ext.Msg.show({
							   title: '系统提示',
					...
					/**订单信息**/
						...
						store.load();
						/**订单修改页面**/
						var win = new Ext.ls.tryOrder.addWin({orderId:record.get('id'),quotationCode:record.get('quotationCode'),supplierId:record.get('supplierId'),updateFlag:true,URL:Ext.ls.tryOrder.updateUrl});
						/**设置窗口的标题**/
						win.setTitle('修改试刀订单');
						/**将store的数据加载到页面**/
						store.on('load',function(){
				...
				listeners: {
					'click' : function(){
							...
							var handleDelete = function (btn){
								var ds = cut_tools.t_order.index_grid.getStore() 
								if(btn == 'ok') {	
									Ext.Ajax.request({
										method: "post",
...
//							}
//							Ext.Ajax.request({
//								url: PATH + '/tryOrder/submitAudit.do',
//								params: { ids: ids },
//									success: function(response){
...
//			},
{
				...
				listeners: {
					'click' : function(){	
							...
							Ext.apply(record.data,{'ownContactPerson':"",'userName':"",'supplierOwnContactPerson':"",'contractCode':""});
						window.open(PATH + '/contractOrder/orderListExcel.do?orderType=6&searchForm='+Ext.util.JSON.encode(record));
					}
		 		}
			},{
				xtype:'tbseparator',
	...
	var  selectForm2 = new Ext.FormPanel({
	        ...
	        items:[
	           ...
	           bbar : ['->',{
			           		...
			           		iconCls : 'icon-search',
			           		handler : function() {
								var store = cut_tools.t_order.index_grid.store;
...
//								store.baseParams.ownContactPerson = form.ownContactPerson;
	           ...
	           items:[
					{items: [{xtype:'textfield',fieldLabel: '订单编号',name: 'orderCode',anchor:'90%'},{xtype:'hidden',fieldLabel: '订单类型',name: 'orderType',value:6}]},
						{items: [{xtype:'textfield',fieldLabel: '报价单',name: 'quotationCode',anchor:'90%'}]},
...
//								store.baseParams.ownContactPerson = form.ownContactPerson;
	...
	cut_tools.t_order.index_win = new Ext.Panel({
            layout: 'border',
            ...
            items: [
            {
				...
				layout: 'fit',
                margins: '-5 5 5 5',
                items : [cut_tools.t_order.index_grid]
            }],
	...
	cut_tools.t_order.index_win.render('t_orders');
