Ext.onReady(function(){
	...
	var OutStockInforListStore = new Ext.data.Store({
		remoteSort : true,
       proxy: new Ext.data.HttpProxy({url:PATH + '/outStock/outStockViewAction.do?ffc=outStockList&outStockType=2,5&limit=' + PAGESIZE}),//调用的动作 
...
var  selectForm3 = new Ext.FormPanel({
                        ...
                        items:[
                            ...
                            items:[
								...
								{items: [{xtype:'textfield',fieldLabel: '报价单编号',name: 'quotationCode',anchor:'100%'}]},	
...
Ext.apply(_config, getConfig());
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        columns: [new Ext.grid.RowNumberer(),//自动行号
			...
			{header: "出库类型", width: 80, sortable: true,  dataIndex: 'outStockType',
				renderer:function(value, cellmeta, record, rowIndex, columnIndex, store){
					var arr = [[5,'提取库存'],
					[2,'<span style="color:green">采购出库</span>']];
					for(var i = 0;i < arr.length ;i++){
						if(value == arr[i][0]){
							return arr[i][1];
						}
			...
			{header: "预定报价编号", width: 170, sortable: true,  dataIndex: 'quotationCode'},
		...
		tbar:[{
				...
				listeners : {
					'click' : function() {
						Ext.ffc.select_quotations('3','6',function(arr,select_quotations_win){
									...
									success: function(response){
											...
											select_quotations_win.close();//关闭报价选择窗口
												var conEditWin = new Ext.ffc.QuotationOutStockEditWindow({			
																		title :  '新增出库单',
																		formTitle:'预定出库',
																		outStockInfor:{quotationId:quotationId,userName:LoginInfor.user.trueName,customerCode:customerCode,customerName:customerName,status:0,outStockType:5,quotationCode:quotationCode,outStockDetails:detail},
																		listeners :{
																			close : function(p){
				...
				listeners : {
					'click' : function() {
							...
							if(arr[0].data.outStockType * 1 == 2){
								Ext.Msg.alert("消息", "预定采购出库单,不允许修改!");
							    return ;
						    }
							var dData = {};
							Ext.apply(dData,arr[0].data);
				...
				listeners : {
					'click' : function(obj) {
						...
						window.open(PATH + "/outStock/outStockOutExcelAction.do?ffc=expertList2Excel&outStockType=2,5&" + para.join("&"));
					}
				}
        	}
		]
	...
	select_quotations_win.render('quotation_out_stock_list_');
