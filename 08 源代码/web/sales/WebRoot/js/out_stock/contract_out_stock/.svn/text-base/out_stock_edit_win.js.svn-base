...
Ext.ffc.ContractOutStockInfoForm = Ext.extend(Ext.FormPanel, {
	constructor : function(_cfg) {
		...
		var f_config = {layout: 'absolute',
						...
						items:[
							...
							{xtype:'textfield',fieldLabel: '制单　人', readOnly : true,value:LoginInfor.user.trueName, name: 'userName',x:340,y:59,width:170},
							//4
		...
		Ext.ffc.ContractOutStockInfoForm.superclass.constructor.call(this, f_config)////-----------------------------Ext.ffc.ContractInfoForm.superclass.constructor.call
	}
	
});

Ext.ffc.ContractOutStockNorthPanel = Ext.extend(Ext.Panel, {
	simpleForm : null,
	constructor : function(_cfg) {
		if(_cfg == null) {
			_cfg = {};
		...
		this.simpleForm = new Ext.ffc.ContractOutStockInfoForm({readOnly : _cfg.readOnly});
		...
		Ext.ffc.ContractOutStockNorthPanel.superclass.constructor.call(this, {
			    region: 'north',
				layout:'fit',
                iconCls:'icon-grid',
                title: '合同出库',
...
Ext.ffc.getContractOutStockEditCenterGridStore = function(jsonData){
 ...
 * @extends Ext.Panel
 */
Ext.ffc.ContractOutStockEditCenterPanel = Ext.extend(Ext.grid.EditorGridPanel, {
	ffc_key : 'Ext.ffc.ContractOutStockEditCenterPanel',
	checkboxObjs : null,
	constructor : function(_cfg) {
		if(_cfg == null) {
			_cfg = {};
		...
		if(_cfg.readOnly){
			delete _cfg.tbar;//一个恶心的办法
		}
		Ext.apply(this, _cfg);
		this.checkboxObjs = new Ext.grid.CheckboxSelectionModel();

		Ext.ffc.ContractOutStockEditCenterPanel.superclass.constructor.call(this, {
			region: 'center',
			layout: 'fit',
			width: 800,
			clicksToEdit:1,//单击修改
	        ...
	        },
			{
				...
				editor: new Ext.form.NumberField({
					...
					validator : function(_value) {
							...
							var _allOutAmount = data.allOutAmount * 1;
							var _orderAmount = data.orderAmount * 1;

							_contractAmount = !Ext.isNumber(_contractAmount) ? 0 : _contractAmount;
							_orderAmount = !Ext.isNumber(_orderAmount) ? 0 : _orderAmount;
							_allOutAmount = !Ext.isNumber(_allOutAmount) ? 0 : _allOutAmount;

							var _needAmount = _contractAmount - _orderAmount - _allOutAmount;
							if(oldAmount < newValue && (newValue - oldAmount) > _needAmount){//数值改大
								return "出库数量不能大于需提数量!";
							}
							...
							var m = newValue * pri;
							record.set("money",m);
							//record.set("allOutAmount",_allOutAmount + (newValue - oldAmount));
							return true;
					},
					listeners : {
...
Ext.ffc.ContractOutStockEditWindow = Ext.extend(Ext.Window, {  
	topPanel : null,
	centerPanel : null,
	outStockInfor : null,
	readOnly : false,
	ContractOutStockEditCenterGridStore : null,
	...
	constructor : function(_cfg) {
		...
		Ext.apply(this, _cfg);
		this.topPanel = new Ext.ffc.ContractOutStockNorthPanel({readOnly:_cfg.readOnly});
		this.ContractOutStockEditCenterGridStore = Ext.ffc.getContractOutStockEditCenterGridStore(this.outStockInfor);
		this.centerPanel = new Ext.ffc.ContractOutStockEditCenterPanel({
			readOnly:_cfg.readOnly,
			ds : this.ContractOutStockEditCenterGridStore,
			store: this.ContractOutStockEditCenterGridStore,
			tbar:[{xtype:'button',text:'参照合同货品',iconCls : 'icon-add',
					handler : function(obj) {
						var grid = obj.ownerCt.ownerCt;
						var editWin = grid.ownerCt;
						var outStockInfor = editWin.outStockInfor;
						try{
						Ext.ffc.select_contract_products(outStockInfor.contractId,
				...
				},{xtype:'button',text:'自动提取库存',iconCls : 'icon-out-stock',
					handler : function(obj) {
						var grid = obj.ownerCt.ownerCt;
						var gridStore = grid.getStore();
						var rowCount = gridStore.getCount();
						...
						for(var i = 0; i < rowCount;i++){
							...
							var _orderAmount = rowData.orderAmount * 1;
							var _allOutAmount = rowData.allOutAmount * 1;
		...
		this.ContractOutStockEditCenterGridStore.load();
		
		Ext.ffc.ContractOutStockEditWindow.superclass.constructor.call(this, {
