...
/**添加产品**/
Ext.ls.scheduleSelfOrder.add = function(btn,e){
	var grid = btn.ownerCt.ownerCt;
	var root = grid.getRootNode(); 
	var loader = grid.getLoader();
	var win = new qsOProductWin({quotationCode:this.quotationCode,supplierId:this.supplierId,
...
/**删除产品**/
Ext.ls.scheduleSelfOrder.deleteProduct = function(btn,e){
	var grid = btn.ownerCt.ownerCt;
...
//						grid.ownerCt.ownerCt.buttons[2].setVisible(false);
//					 }
				},
				scope : this
			});
...
/**统一设置交货期限**/
Ext.ls.scheduleSelfOrder.setupDate = function(btn,e)
{
	var grid = btn.ownerCt.ownerCt;	
...
/**加工订单详细grid**/
	Ext.ls.scheduleSelfOrder.addGrid = Ext.extend(Ext.tree.ColumnTree,{
		quotationCode:null,
		supplierId:null,
		quotationId:null,
		URL:null,//store的URL
		...
		constructor : function(_cfg){
			...
			Ext.ls.scheduleSelfOrder.addGrid.superclass.constructor.call(this, {
					    width:900,
						height: 270,
						rootVisible:false,
						autoScroll:true,
			...
			columns:[
						...
						{header:'报价单分_主键',width:0,dataIndex:'contractProjectSortId',hidden : true}
					],
			tbar :	[{text : "添加",iconCls:'icon-add',handler :Ext.ls.scheduleSelfOrder.add,scope : this,hidden:this.tbarHidden},
					{xtype:'tbseparator',hidden:this.tbarHidden},
					{text : "删除",iconCls : 'icon-delete',handler : Ext.ls.scheduleSelfOrder.deleteProduct,scope:this,hidden:this.tbarHidden},
					{xtype:'tbseparator',hidden:this.tbarHidden},
					{text : '统一设置交货期限',iconCls : 'icon-date',handler :Ext.ls.scheduleSelfOrder.setupDate,scope : this,hidden:this.tbarHidden}],
...
/**添加订单form**/
Ext.ls.scheduleSelfOrder.addForm = Ext.extend(Ext.FormPanel,{
		constructor : function(_cfg){
			if(_cfg == null) {
				_cfg = {};
			}
			...
			this.orderDate = new Ext.form.DateField({
				...
				emptyText:'', allowBlank : false, blankText:'该项为必填项!',disabled : this.isReadOnly,readOnly : true
			});
			...
			var config = [
				//
					...
					{xtype:'label',text: '报价单编号:',x:270,y:5,style:this.lableStyle_},
					{xtype:'textfield',  name: 'quotationCode',readOnly : true,x:370,y:2,width:170},
...
//					{xtype:'textfield',  name: 'status',value:'编制',readOnly : true,x:100,y:62,width:170},
					...
					{xtype:'numberfield', name: 'finalMoney', allowBlank : false, x:630,y:122,width:170,value:0,value:0},
					//6
			...
			Ext.ls.scheduleSelfOrder.addForm.superclass.constructor.call(this, {
				width : 1000,
...
/**加工订单window**/
Ext.ls.scheduleSelfOrder.addWin = Ext.extend(Ext.Window,{
		grid:null,
		form:null,
		quotationId:null,
		quotationCode:null,
		...
		constructor : function(_cfg){
			...
			Ext.apply(this, _cfg);
			this.grid = new Ext.ls.scheduleSelfOrder.addGrid({quotationId:this.quotationId,quotationCode:this.quotationCode,supplierId:this.supplierId,detailFlag:this.detailFlag,URL:this.URL,orderId:this.orderId});
			this.form = new Ext.ls.scheduleSelfOrder.addForm();
			...
			Ext.ls.scheduleSelfOrder.addWin.superclass.constructor.call(this, {
						...
						buttons : [{
							...
							handler : function() {
								...
								Ext.Ajax.request({
								url: PATH + '/selfOrder/addOrder.do?orderType=7',
									...
									success : function(response) {
										var responseArray = Ext.util.JSON.decode(response.responseText); 
										 ...
										 cut_tools.qs_order.index_grid.getStore().reload();
										 } else {
							...
							handler : function() {
								...
								rootCode.eachChild(function(obj){
									var o = Ext.tree.toNewTreeNode(obj.attributes,null,false);
									ss.push(o);
								});
								for(var i=0;i<ss.length;i++)
								{
									var nDate = ss[i].deliveryDate;
									if(nDate == null || nDate == '')
									{
										Ext.Msg.show({title: '系统提示',msg: '产品交货日期不能为空!',width: 300,buttons: Ext.MessageBox.OK,icon: Ext.MessageBox.INFO});
										return;
...
Ext.ffc.contractOrderDetailUpdate = function(obj,ed,value,oldValue){
	...
	ed.editNode.cascade(function(obj){
		 ...
		 {
			oldAmount =  obj.cols['contractAmount'];
			orderAmount = obj.parentNode.cols['orderAmount'];
   ...
   this.show = function(){
       if(this.method != null){
	       this.method();
			...
			store.load();
			/**订单修改页面**/
			var win = new Ext.ls.scheduleSelfOrder.addWin({orderId:this.id,detailFlag:true,URL:Ext.ls.scheduleSelfOrder.detailUrl});
			/**设置窗口的标题**/
			win.setTitle('查看预定加工订单明细');
			/**将store的数据加载到页面**/
