	...
	var qsOContractGrid =  Ext.extend(Ext.grid.GridPanel,{
		store:null,
		constructor : function(_cfg){
			...
			Ext.apply(this, _cfg);
			var proxy = new Ext.data.HttpProxy({url: PATH + "/scheduleOrder/quotationList.do?quotationType=3&leaf=0&outStockType=5"});
			...
			qsOContractGrid.superclass.constructor.call(this, {
				height:500,
				store: this.store,
				columns: [
...
//					{header: "是否预定",dataIndex:'willOrderExpected', width: 100,hidden:true},
	...
	var qsOContractTree = Ext.extend(Ext.tree.ColumnTree,{
		qId:null,
		store:null,
		constructor : function(_cfg){
			if(_cfg == null) {
			...
			qsOContractTree.superclass.constructor.call(this, {
				height: 470,
				bodyStyle:'width:100%',
				rootVisible:false,
				autoScroll:true,
...
//					{header:'价格是否变动',width:100,dataIndex:'priceChange',sortable:true},
	...
	var  qsOContractSelectForm = Ext.extend(Ext.FormPanel, {
		constructor : function(_cfg) {
			if(_cfg == null) {
				_cfg = {};
			}
			Ext.apply(this, _cfg);
			qsOContractSelectForm.superclass.constructor.call(this, {
	...
	var qsOContractWin = Ext.extend(Ext.Window,{
		qsOContractTree:null,
		contract_list:null,
		selectForm2:null,
		constructor : function(_cfg){
			if(_cfg == null) {
			...
			Ext.apply(this, _cfg);
			this.qsOContractTree = new qsOContractTree();
			this.selectForm2 = new qsOContractSelectForm();
			this.contract_list = new qsOContractGrid();
			var _record = null;
			this.contract_list.on('rowclick',function(grid,rowIndex,e) {
						var s = grid.getStore();
						var r = s.getAt(rowIndex);
						_record = r;
						this.qsOContractTree.store.setBaseParam('qId',r.id);
						this.qsOContractTree.store.load({params:{start:0,limit:15,id:r.id}});
				},this);
					...
					var conEditWin = new Ext.ffc.ContractOutStockEditWindow(
						{
							...
							listeners :{
								close : function(p){
									var win = new qsOSupplierWin({quotationId:record.get('id'),contract_code:record.get('quotationCode'),ownContactPerson:record.get('ownContactPerson')});
...
//			this.qsOContractTree.on('dblclick',function(node,e){
//				var record = _record
//				var quotationCode = record.get("quotationCode");
//				var customerName = record.get("customerName");
//				var customerCode = record.get("customerCode");
...
//							listeners :{
//								close : function(p){
//									var win = new qsOSupplierWin({contract_code:record.get('quotationCode'),node:Ext.tree.toNewTreeNode(node.attributes,{},true),ownContactPerson:record.get('ownContactPerson')});
//									var store = win.supplier_grid.getStore();
...
//				},this);
//			},this);
			qsOContractWin.superclass.constructor.call(this, {                                                
				title:"选择报价单",                                                               
				height:500,                                                                     
				width:800,                                                                                                                             
//				modal:true,                                                                     
								...
								var conEditWin = new Ext.ffc.ContractOutStockEditWindow(
									{
										...
										listeners :{
											close : function(p){
												if(node == null)
												{
													var win = new qsOSupplierWin({contract_code:record.get('quotationCode'),ownContactPerson:record.get('ownContactPerson')});
												}
												else
												{
													var win = new qsOSupplierWin({quotationId:record.get('id'),contract_code:record.get('quotationCode'),node:Ext.tree.toNewTreeNode(node.attributes,{},true),ownContactPerson:record.get('ownContactPerson')});
												}
												var store = win.supplier_grid.getStore();
												store.baseParams = {qId:record.get('id'),leaf:0};                   
												store.load({params: {start: 0, limit: 15}}); 
