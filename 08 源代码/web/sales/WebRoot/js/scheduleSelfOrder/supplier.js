	...
	var qsOSupplierGrid = Ext.extend(Ext.grid.GridPanel,{
		store:null,
		constructor : function(_cfg){
			...
			qsOSupplierGrid.superclass.constructor.call(this, {
				height:500,
				store: this.store,
				columns: [
...
var  qsOSupplierSelectForm = Ext.extend(Ext.FormPanel, {
	constructor : function(_cfg) {
		if(_cfg == null) {
			_cfg = {};
		}
		Ext.apply(this, _cfg);
		qsOSupplierSelectForm.superclass.constructor.call(this, {
	...
	var qsOSupplierWin = Ext.extend(Ext.Window,{
		supplier_grid:null,
		contract_code:null,
		selectForm2:null,
		node:null,
		...
		constructor : function(_cfg){
			...
			this.flag = false;
//			alert(this.node);
			this.selectForm2 = new qsOSupplierSelectForm();
			this.supplier_grid = new qsOSupplierGrid();
...
//				var win = new  qsOAddWin({node:this.node,quotationCode:this.contract_code,supplierId:record.get('id'),quotationId:this.quotationId});
				var win = new  qsOAddWin(addParam);
...
//				win.nav1.addTree.supplierId = record.get('id');
			...
			qsOSupplierWin.superclass.constructor.call(this, {
					...
					var record  = this.supplier_grid.getSelectionModel().getSelected();
						if (1!=selectedItem) {
							...
							var addParam = {
									...
									quotationCode:this.contract_code,
									supplierId:record.get('id'),
									URL:Ext.ls.scheduleSelfOrder.addUrl
								};
//							var win = new  qsOAddWin(addParam);
//							win.on('show',function(){
//									win.nav2.addGrid.getForm().loadRecord(contractOrderRecord);
...
//							win.show();
							 ...
							 var win = new  Ext.ls.scheduleSelfOrder.addWin(addParam);
							/**设置窗口的标题**/
							win.on('show',function(){
									win.setTitle('添加预定加工订单');
