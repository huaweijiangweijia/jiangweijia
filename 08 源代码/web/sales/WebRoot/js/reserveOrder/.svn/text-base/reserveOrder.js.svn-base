...
cut_tools.reserve_order.index_grid = null;    
Ext.onReady(function(){
 ...
 function insertTotalRow(){
		var arr = cut_tools.reserve_order.index_grid.getView().getRows();
		if(arr.length){
...
Ext.apply(_config, getConfig());
	...
	var proxy = new Ext.data.HttpProxy({url: PATH + "/reserveOrder/listAction.do"});
	...
	cut_tools.reserve_order.index_grid = new Ext.grid.GridPanel({
		frame:true,
		layout: 'fit',
		sm:gridCheckSele,
		store: store,
...
//			},
		...
		autoExpandColumn: "remark",
			 tbar : [{
				...
				listeners: {
					'click' : function(){
							var record  = cut_tools.reserve_order.index_grid.getSelectionModel().getSelected();
							var arr = gridCheckSele.getSelections();
							if(arr.length == 0){
								Ext.Msg.show({
								   title: '系统提示',
					...
					'click' : function(){
						var arr = gridCheckSele.getSelections();
							...
							var handleDelete = function (btn){
								var ds = cut_tools.reserve_order.index_grid.getStore() 
								if(btn == 'ok') {	
									Ext.Ajax.request({
										method: "post",
										url: PATH + '/reserveOrder/deleteOrder.do',
										params: { ids: ids },
										success: function(response){
											ds.reload();
										}
...
//							}
//							Ext.Ajax.request({
//										url: PATH + '/reserveOrder/submitAudit.do',
//										params: { ids: ids },
//											success: function(response){
...
//			},
			{
				...
				listeners: {
					'click' : function(){
							...
							Ext.Ajax.request({
										url: PATH + '/reserveOrder/placeOrder.do',
										params: { ids: ids },
										success: function(response){
											Ext.Msg.show({
											   title: '系统提示',
...
//			}
	...
	var  selectForm2 = new Ext.FormPanel({
	        ...
	        items:[
	           ...
	           bbar : ['->',{
			           		...
			           		iconCls : 'icon-search',
			           		handler : function() {
								var store = cut_tools.reserve_order.index_grid.store;
	           ...
	           items:[
					{items: [{xtype:'textfield',fieldLabel: '订单编号',name: 'orderCode',anchor:'90%'},{xtype:'hidden',fieldLabel: '订单类型',name: 'orderType',value:2}]},
	...
	cut_tools.reserve_order.index_win = new Ext.Panel({
            layout: 'border',
            ...
            items: [
            {
				...
				layout: 'fit',
                margins: '-5 5 5 5',
                items : [cut_tools.reserve_order.index_grid]
            }],
	...
	cut_tools.reserve_order.index_win.render('reserve_order');
