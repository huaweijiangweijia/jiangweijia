...
Probrand.AddWindow = Ext.extend(Probrand.MangerWindow, {
	constructor : function(_cfg) {
		var arr = checkboxsm.getSelections()[0].data;
		loadProcedureStore(arr.productId);
		if(_cfg == null) {
			_cfg = {};
		}
		Ext.apply(this, _cfg);
		Probrand.AddWindow.superclass.constructor.call(this, {
			...
			buttons : [{
				text : '确认',
				handler : function() {
					var procedureIds = getProcedureIds();
					if (procedureIds == ''){
						Ext.Msg.show({
							title : '提示',
							msg : '请选择领工的工序!',
					...
					arr.procedureId = procedureIds;
					this.ajaxPost(PATH + '/product/produceTaskAction.do?m=doProduceTask', arr);
				},scope : this
...
Probrand.updateMangerWindow = Ext.extend(Ext.Window, {
	brandForm : '',
	constructor : function(_cfg) {
		if (_cfg == null) {
			_cfg = {};
		...
		Ext.apply(this, _cfg);
		Probrand.updateMangerWindow.superclass.constructor.call(this, {
			height : 250,
			width : 380,
			plain : true,
			closeAction : 'hide',
			...
			constrain : true,
			modal : true,
			items : [addNodeForm,procedureGrid]
		});
		this.addEvents("addsuccess");
	},
...
Probrand.ModifyWindow = Ext.extend(Probrand.updateMangerWindow, {
	brandForm : '',
	constructor : function(_cfg) {
		if(_cfg == null) {
			_cfg = {};
		...
		Probrand.ModifyWindow.superclass.constructor.call(this, {
			...
			buttons : [{
				...
				handler : function() {
					...
					arr.procedureId = procedureIds;
					var workTime = addNodeForm.findByType("textfield")[0].getEl().dom.value;
					if (workTime == ''){
						Ext.Msg.show({
							title : '提示',
							msg : '请添加报工工时!',
					...
					this.ajaxPost(PATH + '/product/produceTaskAction.do?m=dailyWork', arr);
				},
				scope : this
...
var checkboxsm = new Ext.grid.CheckboxSelectionModel({header:'',singleSelect:true});//复选框
...
Probrand.BrnadGridPanel = Ext.extend(Ext.grid.GridPanel, {
	...
	constructor : function(_cfg) {
		...
		var ds = new Ext.data.JsonStore({
			...
			autoLoad : false,
			remoteSort : true,
			fields : ['id','productId','planId', 'count','category','status','parentWorkOrderId']
		});
		Probrand.BrnadGridPanel.superclass.constructor.call(this, {
			...
			tbar : [{
				...
				handler : function(){
					var arr = checkboxsm.getSelections();
					var paraArr = [];
...
Ext.onReady(function() {
	...
	brandPanel.render("productWorkOrderView");
})
