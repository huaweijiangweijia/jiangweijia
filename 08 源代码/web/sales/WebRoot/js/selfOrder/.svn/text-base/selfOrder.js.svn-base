...
cut_tools.self_order.index_grid = null;	    
...
Ext.apply(_config, getConfig());
	...
	var proxy = new Ext.data.HttpProxy({url: PATH + "/selfOrder/listAction.do"});
	...
	cut_tools.self_order.index_grid = new Ext.grid.GridPanel({
		frame:true,
		layout: 'fit',
		sm:gridCheckSele,
		store: store,
...
//			},
			{header: "订单附件", width: 60,  dataIndex: 'fileCount',
			 ...
			 tbar : [{
				...
				listeners: {
					'click' : function(){
						var record  = cut_tools.self_order.index_grid.getSelectionModel().getSelected();
				...
				listeners: {
					'click' : function(){
							...
							var handleDelete = function (btn){
								var ds = cut_tools.self_order.index_grid.getStore() 
								if(btn == 'ok') {	
									Ext.Ajax.request({
										method: "post",
...
//							}
//							Ext.Ajax.request({
//										url: PATH + '/selfOrder/submitAudit.do',
//										params: { ids: ids },
//											success: function(response){
				...
				listeners: {
					'click' : function(){	
						var record  =  new Ext.data.Record(selectForm2.getForm().getValues());;
							Ext.apply(record.data,{'userName':"",'ownContactPerson':"",'quotationCode':""});
						window.open(PATH + '/contractOrder/orderListExcel.do?orderType=3&searchForm='+Ext.util.JSON.encode(record));
					}
		 		}
			},{
				xtype:'tbseparator',
...
Ext.ffc.showEditResservePlanWindow = function(process_orderId,ReservePlanListStore){
	...
	var  selectForm2 = new Ext.FormPanel({
	        ...
	        items:[
	           ...
	           bbar : ['->',{
			           		...
			           		iconCls : 'icon-search',
			           		handler : function() {
								var store = cut_tools.self_order.index_grid.store;
	           ...
	           items:[
					{items: [{xtype:'textfield',fieldLabel: '订单编号',name: 'orderCode',anchor:'90%'},{xtype:'hidden',fieldLabel: '订单类型',name: 'orderType',value:3}]},
	...
	cut_tools.self_order.index_win = new Ext.Panel({
            layout: 'border',
            ...
            items: [
            {
				...
				layout: 'fit',
                margins: '-5 5 5 5',
                items : [cut_tools.self_order.index_grid]
            }],
		...
		cut_tools.self_order.index_win.render('self_order');
