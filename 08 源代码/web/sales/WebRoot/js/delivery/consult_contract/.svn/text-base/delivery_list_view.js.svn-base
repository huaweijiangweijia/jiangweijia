Ext.onReady(function(){
	...
	var deliveryListStore = new Ext.data.Store({
	   remoteSort : true,
       proxy: new Ext.data.HttpProxy({url:PATH + '/delivery/deliveryViewAction.do?ffc=viewList&deliveryType=0&limit=' + PAGESIZE}),//调用的动作 
...
var  selectForm3 = new Ext.FormPanel({
                        ...
                        items:[
                            ...
                            items:[
								...
								{columnWidth:0.30,items: [new Ext.ffc.DeliveryStatusComboBox({anchor:'60%'})]},        
						...
						bbar : ['->',{xtype:'button',text: '搜    索',iconCls:'icon-search',name: 'seachBt',width:80,
									handler : function() {
										var p = selectForm3.getForm().getValues();
										for(var i in p){
											deliveryListStore.setBaseParam(i, p[i]);
										}
...
Ext.apply(_config, getConfig());
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        columns : [new Ext.grid.RowNumberer(),
			...
			{header: "订单编号", width: 155, sortable: true,  dataIndex: 'orderCode'},
		...
		tbar:[{
				...
				listeners : {
					'click' : function() {
						Ext.ffc.select_contracts(function(arr,select_quotations_win){
							...
							var fileCount = arr[0].get("fileCount");
							var cusLockStatus = arr[0].get('cusLockStatus');
							if(cusLockStatus * 1 == 0){
								Ext.Msg.alert("消息", "所选中合同,所属客户已经锁定,不允许交货!");
								return ;
							}
							if(fileCount * 1 == 0){
								Ext.Msg.alert("消息", "所选中合同,未上传合同文本,不允许交货!");
								return ;
							}
							...
							Ext.Ajax.request({
									method: "post",
									url: PATH + '/delivery/deliveryEditAction.do?ffc=consultContract&id=' + contractInforId,
				...
				listeners : {
					'click' : function(obj) {
						var arr = gridCheckSele.getSelections();
						    ...
						    if(status != 2){
								Ext.Msg.alert("消息", "只有确认后的交货单，才允许导出!");
							    return ;
							}*/
						var data = arr[0].data;
				...
				listeners : {
					'click' : function(obj) {
						...
						window.open(PATH + "/delivery/outExcelAction.do?ffc=expertList2Excel&deliveryType=0&" + para.join("&"));
					}
				}
        	}
		]
	...
	select_quotations_win.render('delivery_list_');
