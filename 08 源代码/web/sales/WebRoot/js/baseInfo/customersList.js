				...
				Ext.zhj.CustomersDegreeCombox.superclass.constructor.call(this,
						{
							...
							listeners : {
								...
								'select' : function(_combox, _record) {
									this.degreeId=_record.get('id');
								}
							}
						})
			}
 ...
 * 
 */
Ext.ffc.ReputationPeriodComboBox = Ext.extend(Ext.form.ComboBox, {
	store : null,
	constructor : function(_cfg) {
		if (_cfg == null) {
			_cfg = {};
		...
		this.store = new Ext.data.SimpleStore({
					fields : ['reputationPeriod', 'value'],
					data : [['10天', '10'], ['15天', '15'],['20天','20'],['25天','25'],['30天','30']]
				});
		Ext.ffc.ReputationPeriodComboBox.superclass.constructor.call(this, {
					fieldLabel : '信用有效期',
					hiddenName : 'reputationPeriod',
					mode : 'local',
					displayField : 'reputationPeriod',
					valueField : 'value',
					// anchor:'90%',
					readOnly : true,
					frame : true,
		...
		Ext.zhj.CustomersGrid.superclass.constructor.call(this, {
			...
			tbar : [{
						...
						handler : function() {
							...
							if (selectedCount == 0) {
								...
								updateCustomersWindow.on('customerAddSuccess',function(){
									this.getStore().reload();								
								},this);
						...
						listeners : {
							'click' : function(obj) {
								var values = searchCustomersForm.getForm().getValues();
								var para = [];
								for(var i in values){
								...
								var path = PATH + "/baseInfo/customersOutExcelAction.do";
								if(para.length>0){
									path+= para.join("&")
								}
								window.open(path);
			...
			cm : new Ext.grid.ColumnModel([
					...
					new Ext.grid.RowNumberer({
						...
						width : 80,
						editor : new Ext.ffc.ClosingAccountModeComboBox()
					},{
						header : '联系人1',
						sortable : true,
						...
						editor : {
							...
							listeners:{
								'select':function(combo,record,index ){ 
										this.ownContactPersonId = record.data.id;
									},scope:this
							}
						}
...
Ext.onReady(function() {
	...
	searchCustomersForm = new Ext.zhj.SearchCustomers();
...
Ext.zhj.AddCustomersWindow = Ext.extend(Ext.Window, {
	...
	constructor : function(_cfg) {
		...
		this.ownContactPersonCombox = new Ext.zhj.OwnContactPersonCombox({width : 140});
		this.reputationTypeCombox = new Ext.zhj.ReputationTypeCombox({width : 140});
		this.lockStatus = new Ext.ffc.LockStatusComboBox({width : 140});
		this.reputationPeriod = new Ext.ffc.ReputationPeriodComboBox({width : 140});
		Ext.zhj.AddCustomersWindow.superclass.constructor.call(this, {
					bodyStyle : 'width:100%',
					title : this['title'] == null ?"新增客户信息" : this['title'],
					height : 410,
					width : 600,
					plain : true,
					...
					items : [{
								...
								items : [{
											...
											items : [{
														...
														allowBlank : false,
														anchor : '90%',
														readOnly:this.isModify,
														style:this.isModify?'color:#cecece;':''
													}]
										}, {
											items : [{
														xtype : 'textfield',
											...
											items : [this.ownContactPersonCombox]
												...
												items : [new Ext.ffc.ClosingAccountModeComboBox({width : 140})]
					...
					buttons : [{
						...
						handler : function() {
							...
							var _str = addForm.getValues();
							var _record = new Ext.data.Record(_str);
							Ext.apply(_record.data, {"customerDegreeId" : _addCustomersWindow.customersDegreeCombox.degreeId})
							var customersFormInfo = Ext.util.JSON.encode(_record.data);
							Ext.Ajax.request({
										url : PATH + _formUrl,
										params : {
