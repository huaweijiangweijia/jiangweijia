...
public class MD5 {
	...
	private static char bytBuffer[] = new char[64];
	private static long lngState[] = new long[4];
	private static long lngByteCount = 0;
	...
	private static void update(char bytInput[], long lngLen) {
		int index = (int) (lngByteCount % 64);
		int i = 0;
		lngByteCount += lngLen;
		...
		if (lngLen >= partLen) {
			for (int j = 0; j < partLen; ++j) {
				bytBuffer[j + index] = bytInput[j];
			}
			transform(lngState, bytBuffer);

			for (i = partLen; i + 63 < lngLen; i += 64) {
				for (int j = 0; j < 64; ++j) {
					bytBuffer[j] = bytInput[j + i];
				}
				transform(lngState, bytBuffer);
			}
			index = 0;
		...
		for (int j = 0; j < lngLen - i; ++j) {
			bytBuffer[index + j] = bytInput[i + j];
		}
	...
	public static String getMD5(String passWord) {
		init();
		char[] chrTestData = passWord.toCharArray();
	...
	// System.out.println("MD5 Speed Test: " + time2 + "sec = " +
	// md5Test.toHexString() );
	//
	// }
}
