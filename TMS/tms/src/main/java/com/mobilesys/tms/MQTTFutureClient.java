...
public class MQTTFutureClient {
	private static final Logger LOG = Logger.getLogger(MQTTFutureClient.class);
	private final static String CONNECTION_STRING = "tcp://182.92.155.208:21613";
	private final static boolean CLEAN_START = true;
	private final static short KEEP_ALIVE = 30;// 低耗网络，但是又需要及时获取数据，心跳30s
	private final static String CLIENT_ID = "reveiveService1";
	...
	public static void main(String[] args) {
		...
		try {
			...
			connection.connect();
			Topic[] topics = { new Topic("china/shanghai", QoS.AT_LEAST_ONCE) };
			connection.subscribe(topics);
			while (true) {
				Future<Message> futrueMessage = connection.receive();
				Message message = futrueMessage.await();

				System.out.println("MQTTFutureClient1.Receive Message " + "Topic Title :" + message.getTopic()
